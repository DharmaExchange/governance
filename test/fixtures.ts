...
import Uni from '../build/Dharma.json'
...
export async function governanceFixture(
  ...
  const dharma = await deployContract(wallet, Uni, [wallet.address, timelockAddress, now + 60 * 60])
  ...
  const governorAlpha = await deployContract(wallet, GovernorAlpha, [timelock.address, dharma.address])
  expect(governorAlpha.address).to.be.eq(governorAlphaAddress)

  return { dharma, timelock, governorAlpha }
}
